...
export default async (lineReader: any, params: Params) => {
  ...
  const complexity = (keyboardCodeLength: number, keypadCode: string): number =>
    keyboardCodeLength * Number(keypadCode.replaceAll('A', ''))

  const solveFor = (codes: string[], numberOfRobots: number): number =>
    codes.reduce((acc, code) => {
      let finalCodeSize: number = doKeypadMovesFor(code).reduce(
        (acc, c) => acc + doKeyboardMovesFor(c, numberOfRobots, {}),
        0
      )
