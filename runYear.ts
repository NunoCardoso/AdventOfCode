...
Promise.all(puzzles.map(getConfig)).then(async (days: Array<any>) => {
  while(days.length > 0) {
    ...
    let durString = dur < 1 ? clc.green(dur + 's') :
      dur < 3 ? clc.yellow(dur + 's') : clc.red(dur + 's')
    ...
    if (res.config?.tags) status += 'Tags: ' +
      (Array.isArray(res.config?.tags) ? res.config?.tags.join(', ') : res.config?.tags) + '\n'
