...
export default async (lineReader: any, params: Params) => {
  ...
  const printWorld = (
    { world, opened, iteration, letter }:
      {world: World, opened: Paths, iteration: number, letter: Record<string, string>}
  ) => {
    const m: string = !_.isEmpty(opened) ? opened[0].head.move.toString() : '-'
    console.log('      ' + _.range(0, world.columns).map((i: number) =>
      Math.floor(i / 100).toString()
    ).join(''))
    console.log('m ' + m.padStart(2, '0') + ' ' + _.range(0, world.columns).map((i: number) =>
      Math.floor((i % 100) / 10).toString()
    ).join(''))
    console.log('i ' + (iteration + 1).toString().padStart(2, '0') + ' ' +
      _.range(0, world.columns).map((i: number) =>
        (i % 10).toString()
      ).join(''))
  ...
  const getDistance = (point1: Point, point2: Point): number =>
    Math.sqrt((point1[0] - point2[0]) * (point1[0] - point2[0]) +
  ...
  const makeNewPaths = ({ paths, world }: {paths: Paths, world: World}): Paths => {
    ...
    paths.forEach((path: Path) => {
      ...
      log.trace('newPaths before', newPaths.map((x: Path) => printPath(x)))
      let _newPaths: Paths = [{
        head: {
      ...
      _newPaths = _.reject(_newPaths, (newPath: Path) => {
        ...
        if (newPath.head.point[0] < 0 || newPath.head.point[1] < 0 ||
        ...
        if (worldMove[newPath.head.point[0]][newPath.head.point[1]] !== '.') {
          log.trace('rejecting new path', printPath(newPath), 'on moves #', newMove,
            'value is', worldMove[newPath.head.point[0]][newPath.head.point[1]])
          return true
        }
      ...
      log.trace('generated _newPaths', _newPaths.map((x: Path) => printPath(x)))
      log.trace('newPaths for ', printPath(path), 'is now', newPaths.map((x: Path) => printPath(x)))
    })
    return newPaths
  }
  ...
  const searchAlgorithm = async ({ opened, finished, world, end, iteration, endStage }: {
    opened: Paths, finished: Paths, world: World, end: Point, iteration: number, endStage: number
  }) => {
    ...
    // sort and prune
    opened.sort((a: Path, b: Path) => a.head.distance - b.head.distance)
      .splice(20, 100)

    log.trace(opened)
    if (params.ui?.show && params.ui?.during) {
      printWorld({
        world, opened: opened, iteration, letter: { path: 'S', opened: 'O', new: '@' }
      })
  ...
  while (_.isEmpty(finished)) {
    await searchAlgorithm({
      opened: opened, finished, world, end, iteration, endStage: 1
    })
    iteration++

    if (iteration % 10 === 0) {
      log.debug(iteration, 'opened', opened.length, 'finished',
  ...
  while (_.isEmpty(finished)) {
    await searchAlgorithm({
      opened: opened, finished, world, end, iteration, endStage: 3
    })
    iteration++
