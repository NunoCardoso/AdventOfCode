...
export default async (lineReader: any, params: Params) => {
  ...
  type Resources = {
    ore: number, clay: number, obsidian: number, geode: number,
  }
  ...
  const haveResourcesForGeodeRobot = (blueprint: Blueprint, head: Step) =>
    head.resources.obsidian >= blueprint.geodeRobot.obsidian &&
    head.resources.ore >= blueprint.geodeRobot.ore

  const haveResourcesForObsidianRobot = (blueprint: Blueprint, head: Step) =>
    head.resources.clay >= blueprint.obsidianRobot.clay &&
    head.resources.ore >= blueprint.obsidianRobot.ore
  ...
  const timeToMakeAGeodeRobot = (blueprint: Blueprint, step: Step): number =>
    1 + (haveResourcesForGeodeRobot(blueprint, step)
      ? 0
      : Math.max(
        Math.ceil((blueprint.geodeRobot.obsidian - step.resources.obsidian) / Math.max(step.robots.obsidian, 1)),
  ...
  const timeToMakeAnObsidianRobot = (blueprint: Blueprint, step: Step): number =>
    1 + (haveResourcesForObsidianRobot(blueprint, step)
      ? 0
      : Math.max(
  ...
  const timeToMakeAClayRobot = (blueprint: Blueprint, step: Step): number =>
    1 + (haveResourcesForClayRobot(blueprint, step)
      ? 0
      : Math.max(
        Math.ceil((blueprint.clayRobot.ore - step.resources.ore) / step.robots.ore)
      ))

  const timeToMakeAnOreRobot = (blueprint: Blueprint, step: Step): number =>
    1 + (haveResourcesForOreRobot(blueprint, step)
      ? 0
      : Math.max(
        Math.ceil((blueprint.oreRobot.ore - step.resources.ore) / step.robots.ore)
      ))
  ...
  const printStep = (s: Step) => '[' + s.robots.ore.toString() + ',' + s.robots.clay.toString() + ',' +
    s.robots.obsidian.toString() + ',' + s.robots.geode.toString() + '](' + s.time + ')'

  const printPath = (path: Path) => path.map(printStep).join(',')

  const getThemGeodes = ({ path, finished, blueprint, maxMinutes }:
    {path: Path, finished: Finished, blueprint: Blueprint, maxMinutes: number }) => {
    ...
    const searchAlgorithm = ({ path, finished, blueprint, maxMinutes }:
      {path: Path, finished: Finished, blueprint: Blueprint, maxMinutes: number }) => {
      ...
      if (canBuildGeodeRobot(blueprint, head)) {
        ...
        searchAlgorithm({
          path: path.concat(newStep), finished, blueprint, maxMinutes
        })
      }
