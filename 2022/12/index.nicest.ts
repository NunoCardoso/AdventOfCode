...
export default async (lineReader: any, params: Params) => {
  ...
  const getDistanceToFinish = (path: Partial<Point>, end: Point): number =>
    Math.sqrt((path.x! - end.x) * (path.x! - end.x) + (path.y! - end.y) * (path.y! - end.y))
  ...
  const printData = (
    ...
    newPoints: Points,
    visited: Points,
    finished: Points | undefined
  ): Array<string> => {
    console.log(
      '   ' +
        _.range(0, world[0].length)
          .map((i: number) =>
    ...
    console.log(
      '   ' +
        _.range(0, world[0].length)
          .map((i: number) => (i === path[path.length - 1].y ? clc.red(i % 10) : i % 10))
          .join('')
    )
    ...
    return _world.map(
      (m, i) =>
        (i === path[path.length - 1].x
          ? clc.red(i.toString().padStart(2, '0'))
  ...
  const makeNewPaths = (path: Points, opened: Paths, visited: Points, finished: Points | undefined) => {
    ...
    let newPoints: Points = [
      {
        ...
        y: head.y,
        cost: head.cost + 1,
        distance: getDistanceToFinish({ x: head.x - 1, y: head.y }, end)
      },
      {
        x: head.x + 1,
        y: head.y,
      ...
      },
      {
        ...
        y: head.y - 1,
        cost: head.cost + 1,
        distance: getDistanceToFinish({ x: head.x, y: head.y - 1 }, end)
      },
      {
        x: head.x,
        y: head.y + 1,
    ...
    newPoints = _.reject(newPoints, (newPoint: Point) => {
      ...
      const levelDifference: number =
        getHeight(world[newPoint.x][newPoint.y]) - getHeight(world[head.x][head.y])
      ...
      if (matchOpenedPathIndex >= 0) {
        if (opened[matchOpenedPathIndex][opened[matchOpenedPathIndex].length - 1].cost <= newPoint.cost) {
          ...
          log.debug(
            'new path',
            printPoint(newPoint),
            'is better than an opened path, deleting that with head',
      ...
      if (matchVisitedPathIndex >= 0) {
        if (visited[matchVisitedPathIndex].cost <= newPoint.cost) {
          ...
          log.debug(
            'new path',
            printPoint(newPoint),
            "is better than a visited path, let's see where it goes"
          )
        }
      }
      ...
      if (finished) {
        const matchFinishedPathIndex: number = _.findLastIndex(finished, (p: Point) => isSame(p, newPoint))
        if (matchFinishedPathIndex >= 0 && newPoint.cost < finished[matchFinishedPathIndex].cost) {
          ...
          log.debug(
            'new path',
            printPoint(newPoint),
            'has better tail than finished: than an opened path, deleting that with head'
          )
          finished = path.concat(finished.slice(matchFinishedPathIndex, finished.length))
  ...
  const searchAlgorithm = async (opened: Paths, visited: Points, finished: Points | undefined) => {
    ...
    log.debug(
      ...
      opened.length,
      'finished',
      finished ? finished[finished.length - 1].cost : '-'
    )
    ...
    log.debug(
      ...
      opened.slice(-3).map((p) => JSON.stringify(p[p.length - 1]))
    )
  ...
  if (params.part1?.skip !== true) {
    ...
    if (params.ui?.show && params.ui?.end) {
      printData(world, finished!, [], [], [], undefined).forEach((line) => console.log(line))
    }
  }
